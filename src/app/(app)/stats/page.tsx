    ...
    <>
      ...
      <section className="stats-hero card">
        <div className="stats-hero-main">
          <p className="hint-compact">{t(locale, "stats.gameHubBalance")}</p>
          <p className="stats-hero-points">{userPoints.totalPoints}</p>
          <div className="btn-row">
            <span className="chip">{t(locale, "stats.weeklyPoints")}: {userPoints.weeklyPoints}</span>
            <span className="chip">{t(locale, "stats.currentStreak")}: {streakStats.currentStreak}</span>
            <span className="chip">{t(locale, "route.streakMultiplier", { multiplier: userPoints.streakMultiplier })}</span>
          </div>
        </div>
        <div className="stats-hero-side">
          <h3>{t(locale, "stats.gameHubGolden")}</h3>
          {goldenToday.length === 0 ? (
            <p className="hint-compact">{t(locale, "game.goldenEmpty")}</p>
          ) : (
            <ul className="dense-list">
              {goldenToday.slice(0, 4).map((g) => {
      ...
      <div className="stats-leaderboard-row">
        <div className="card stats-leaderboard-card">
          ...
          {pointsLeaderboard.length === 0 ? (
            ...
            <ol className="dense-list">
              {pointsLeaderboard.slice(0, 8).map((entry) => (
      ...
      <div className="card">
        ...
        <div className="stats-tier-grid">
          {achievements.scalable.map((a) => {
            ...
            return (
              <div key={a.category} className="achievement-badge">
                <span

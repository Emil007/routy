...
import { getNode, deleteNode } from "@/lib/nodes";
import { nodeUsedByActiveRoute } from "@/lib/activeRoute";
...
export async function POST(request: Request) {
  ...
  if (nodeUsedByActiveRoute(node.id)) return NextResponse.json({ error: "node_active" }, { status: 409 });

  deleteNode(node.id);
  logActivity(user.id, "delete", "node", node.id, { name: node.name });

...
export function getNetworkVersion(): string {
  const row = db
    .prepare(
      `SELECT
         ...
         (SELECT COALESCE(MAX(created_at), '') FROM nodes) AS node_created,
         (SELECT COALESCE(MAX(created_at), '') FROM segments) AS segment_created`,
    )
    .get() as {
    node_count: number;
  ...
  const payload = `${row.node_count}:${row.segment_count}:${row.max_node_id}:${row.max_segment_id}:${row.node_created}:${row.segment_created}`;
